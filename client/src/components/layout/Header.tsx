    ...
    <header className="backdrop-blur-sm bg-darkBg/90 fixed w-full z-50 border-b border-[hsl(var(--solana-purple))/20]">
        ...
        <nav className="hidden md:flex space-x-1">
          {tabs.map((tab) => (
            <Link key={tab.path} href={tab.path}>
              <div className={`px-4 py-2 rounded-md text-white hover:bg-white/10 transition-all cursor-pointer ${location === tab.path ? 'bg-white/10' : ''}`}>
                {tab.name}
              </div>
            </Link>
          ))}
      ...
      <div className="md:hidden border-t border-[hsl(var(--solana-purple))/20] overflow-x-auto">
        <div className="flex space-x-1 px-4 py-2">
          {tabs.map((tab) => (
            <Link key={tab.path} href={tab.path}>
              <div className={`flex-1 px-4 py-2 rounded-md text-white hover:bg-white/10 transition-all text-sm text-center cursor-pointer ${location === tab.path ? 'bg-white/10' : ''}`}>
